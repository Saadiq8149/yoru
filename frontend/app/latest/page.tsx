...
export default function LatestPage() {
  ...
  const fetchLatest = async (page: number = 1, append: boolean = false) => {
    try {
      ...
      const response = await fetch(`/api/latest?page=${page}&per_page=20`);
  ...
  const loadMore = () => {
    if (hasNextPage && !loadingMore) {
      fetchLatest(currentPage + 1, true);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground">Loading latest anime...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        {!hasNextPage && anime.length > 0 && (
          <div className="text-center mt-8 text-muted-foreground">
            You've reached the end of latest anime!
          </div>
        )}
      </div>
    </div>
